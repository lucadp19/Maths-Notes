...
\begin{lemma}{ord_classilat=ord_sgr}
...
\begin{proof}[Dimostrazione del \Cref{th:lagrange}]
...
\begin{proposition}{cond_laterale_è_sgrp}
...
\begin{corollary}[Corollario al Teorema di Lagrange] {lagrange}
...
\begin{proof}
    ...
    Dunque $G$ è ciclico e per il \Cref{th:iso_ciclico} è isomorfo a $\Zmod{p}$.
\end{proof}

Il teorema di Lagrange ci consente inoltre di dimostrare molto semplicemente il Teorema di Eulero-Fermat.
\begin{proof}
    Segue dal \Cref{cor:lagrange} (in particolare dal \hyperref[cor:x_alla_ordG=e_G]{punto (ii)}) considerando come gruppo $(\units{\Zmod{n}}, \cdot)$: infatti per definizione $\phi(n) = \abs*{\units{\Zmod{n}}}$, da cui la tesi.
\end{proof}
...
\begin{proposition} {normale_sse_chiuso_per_coniugio}
...
\begin{proof}
    \begin{enumerate}[label={(\roman*)}]
        \item Per la \Cref{prop:normale_sse_chiuso_per_coniugio} la tesi è equivalente a dimostrare che \[
...
\begin{proof}
    ...
    Inoltre per definizione \begin{align*}
        ...
        &= N,  
    \end{align*} dove il secondo segno di uguaglianza viene dalla \Cref{prop:cond_laterale_è_sgrp} (in particolare per l'equivalenza tra i punti (ii) e (iii)).
\end{proof}
...
\end{corollary}
\begin{proof}
    Infatti se $N \normal G$ allora per la \Cref{prop:kernel_proiezione} segue che $N = \ker \pi_N$; invece dato un omomorfismo di gruppi $\phi : G \to G^\prime$ vale che $\ker \phi$ è normale per la \Cref{prop:rel_kernel_sgr_normali}.
\end{proof}
...
\begin{proof}
    ...
    \paragraph{Caso base}
    Se $G$ ha ordine $p$, allora per il \Cref{cor:cardG=p_implies_cyclic_G} segue che $G \isomorph \Zmod{p}$, dunque ogni elemento invertibile ha ordine $p$.

    \paragraph{Passo induttivo} Supponiamo $n > p$,  $p \divides n$.

    Se $G$ è ciclico, allora $G \isomorph \Zmod{n}$. Per il \Cref{cor:cons_ord_in_Z/nZ} sappiamo che ci sono $\phi(p) = p-1$ elementi di ordine $p$ in $\Zmod{n}$, dunque in particolare vi è almeno un elemento di ordine $p$.
    ...
    (poiché $\gen*{g}$ è ciclico continua a valere la proposizione \Cref{cor:cons_ord_in_Z/nZ}) e quindi in $G$.
    ...
    Essendo $\pi_H$ surgettiva, tale elemento sicuramente esiste (anche se non è detto che sia unico). Per la \Cref{prop:omo_ord} segue che  \[
...
\begin{proposition}{G/Z(G)_cyclic=>abelian_G}
