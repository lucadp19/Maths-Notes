...
\end{corollary}
\begin{proof}
    Siano $p, q \in A[X] \setminus \set*{0_{A[X]}}$, con $\deg p = n \geq 0$, $\deg q = m \geq 0$. Allora per la \Cref{prop:deg_somma_prod_polinomi} vale che \[
...
\begin{proof}
    ...
    Notiamo che $p, q \neq 0_{A[X]}$. Infatti se uno dei due fosse il polinomio nullo per la \Cref{prop:prop_anelli:per_0} il loro prodotto dovrebbe essere il polinomio nullo e non l'unità. Allora esistono $\deg p, \deg q \geq 0$ e vale che \[
...
\end{proposition}
\begin{proof}
    La dimostrazione è uguale alla dimostrazione della \Cref{prop:primo_sse_irr_in_Z}.
\end{proof}
